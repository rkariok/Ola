...
export const calculateMaxPiecesPerSlab = (pieceW, pieceH, slabW, slabH, kerfWidth) => {
  const kerf = kerfWidth || 0;  // Use kerfWidth directly, no toggle needed
  let maxPieces = 0;
...
export const calculateProductResults = (product, stoneOptions, settings) => {
  ...
  const maxPiecesPerSlab = calculateMaxPiecesPerSlab(
    w, d, slabWidth, slabHeight, settings.kerfWidth
  );

...
export const sendQuoteEmail = async (userInfo, allResults, stoneOptions) => {
  ...
  try {
            ...
            ${allResults.map((p, i) => `
              <div style="border-bottom: 1px solid #e5e7eb; padding: 12px 0;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                  <div>
                    <strong style="color: #1f2937;">${p.customName || `Type ${i + 1}`}</strong>
    ...
    const templateParams = {
      ...
      products_list: allResults.map((p, i) => 
        `- ${p.customName || `Type ${i + 1}`}: ${p.stone} ${p.width}"×${p.depth}" (Qty: ${p.quantity}) - $${p.result?.finalPrice?.toFixed(2) || '0.00'}`
      ).join('\n'),
